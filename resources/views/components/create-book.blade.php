            ...
            <div class="modal-body">
                <form class="w-60 p-3 has-required" action="{{url('/book')}}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="form-row justify-content-around">
                        <div class="form-group">
                            <button class="btn btn-success dropdown-toggle required" type="button"
                            ...
                            <div class="dropdown-menu">
                                @foreach($authors as $author)
                                    <label>
                                        <input required type="checkbox" name="author_ids[]" value="{{$author->id}}" />
                            ...
                            <div class="dropdown-menu">
                                @foreach($genres as $genre)
                                    <label class="required">
                                        <input required type="checkbox" name="genre_ids[]" value="{{$genre->id}}" />
                                        {{$genre->name}}
                                    </label>
                                @endforeach
                            </div>
